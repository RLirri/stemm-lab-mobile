...
export default function A6ReflectionSubmitScreen({route, navigation}: Props) {
    ...
    const viewModel = useMemo(() => {
        ...
        const hasAnyReaction = (draft.reactionTrials ?? []).some((t) => isFiniteNumber(t?.reactionTimeMs));
        const hasAnyTrace = (draft.tracingResults ?? []).some((r) => isFiniteNumber(r?.accuracyScorePct));

        const latestTracingOverall =
            [...(draft.tracingResults ?? [])].sort((a, b) => (b.endedAt ?? 0) - (a.endedAt ?? 0))[0];
            ...
            <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
                ...
                <View style={styles.card}>
                    ...
                    <View style={{marginTop: 10, gap: 10}}>
                        <ChecklistRow label="Reaction dataset recorded (required)" ok={viewModel.reactionOk}/>
                        <ChecklistRow label="Tracing result recorded (required)" ok={viewModel.tracingOk}/>
